...
const _generateCustomerSession = (user) => {
    ...
    const accessTokenPayload = {
        ...
        role: user.role.role,
        profileImageUrl: user.profileImageUrl,
        ...(isSupplier && { isVerified: finalIsVerified }),
        isActive: user.isActive
    };

    const accessToken = generateAccessToken(accessTokenPayload);
    const refreshToken = generateRefreshToken({ userId: user.userId });
    return { userProfile: accessTokenPayload, accessToken, refreshToken };
};
...
const verifyOtpAndCheckUser = async (phoneNumber, otp) => {
    ...
    if (!verificationRecord)
        throw {
            code: 400,
            message: "No OTP was sent to this number. Please request one first."
        };
    if (verificationRecord.verified)
        throw { code: 400, message: "This OTP has already been used." };
    ...
    if (!isMatch)
        throw { code: 400, message: "The OTP you entered is incorrect." };
    ...
    if (user) {
        ...
        return {
            ...
            data: {
                userExists: false,
                message:
                    "Phone number verified. Please complete your registration."
            }
        };
    }
};
...
const quickRegisterCustomer = async (payload) => {
    ...
    if (!customerRole)
        throw {
            code: 500,
            message: "Customer role not configured in the system."
        };
    ...
    return {
        success: true,
        code: 201,
        message: "Registration successful. Welcome to MayaVriksh!",
        data: { ...session }
    };
};
